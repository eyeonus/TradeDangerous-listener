...
from .listener_env import ListenerEnv
from .listener import Listener

def db_execute(env, db, sql_cmd, args = None):
    cur = db.cursor()
    success = False
    result = None
    while env.go and not success:
        try:
            if args:
...
def get_messages(env, config):
    listener = Listener(env, config)
    listener.get_batch(env.q)

def check_update(env, config):
    ...
    while env.go:
        ...
        if Path.exists(env.eddbPath / listings_path):
            localModded = (env.eddbPath / listings_path).stat().st_mtime
...
def process_messages(env, config):
    ...
    while env.go:
        ...
        if env.update_busy or env.export_busy:
            print("Message processor acknowledging busy signal.")
            env.process_ack = True
            while (env.update_busy or env.export_busy) and env.go:
                time.sleep(1)
            env.process_ack = False
        ...
        station_id = env.station_ids.get(system + "/" + station)
        if not station_id:
            # Mobile stations are stored in the dict a bit differently.
            station_id = env.station_ids.get("MEGASHIP/" + station)
            system_id = env.system_ids.get(system)
        ...
        if config['debug']:
            with env.debugPath.open('a', encoding = "utf-8") as fh:
        ...
        for commodity in commodities:
            ...
            item_edid = env.db_name.get(commodity['name'].lower())
            if not item_edid:
                if config['verbose']:
            ...
            item_id = env.item_ids.get(item_edid)
            if not item_id:
                if config['verbose']:
...
def export_listings(env, config):
    """
        ...
        while env.go:
            ...
            while time.time() < now + config['export_every_x_sec']:
                ...
                if env.update_busy:
                    ...
                    env.export_ack = True
                    while env.update_busy and env.go:
                        time.sleep(1)
                    env.export_ack = False
                    # Just in case we caught the shutdown command while waiting.
                    if not env.go:
                        break
                ...
                if time.time() >= maintenance_time:
                    ...
                    try:
                        db_execute(env, db, "VACUUM")
                        db_execute(env, db, "PRAGMA optimize")
                    except Exception as e:
            ...
            while not (env.process_ack):
                if not env.go:
                    break
            print("Busy signal acknowledged, getting listings for export.")
            try:
                results = list(fetchIter(db_execute(env, db, "SELECT * FROM StationItem WHERE from_live = 1 ORDER BY station_id, item_id")))
            except sqlite3.DatabaseError as e:
                print(e)
                env.export_busy = False
                continue
            env.export_busy = False
...
def main():
    ...
    listener_thread = threading.Thread(target=get_messages, args=[env, env.config])
    update_thread = threading.Thread(target=check_update, args=[env, env.config])
    process_thread = threading.Thread(target=process_messages, args=[env, env.config])
    export_thread = threading.Thread(target=export_listings, args=[env, env.config])
    ...
    env.eddbPath = plugins.eddblink_plug.ImportPlugin(tdb, tradeenv.TradeEnv()).dataPath.resolve()
    env.debugPath = env.eddbPath / Path("debug.txt")
    env.refresh_dicts()
